...
[•]-BOT VERSION : 1.0.0
[•]-DONO : SAMODS
[•]-DEDICADO A EQUIPE SUZANO VIVEIRO
|< ================================================== >|`, {
  font: 'console',
  align: 'center',
...
var qrcode = "./bot.json" //./${sessionName}.json`
...
require('./funções de cmd/menu/menu')
  ...
  nocache('./funções de cmd/menu/menu', Módulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(`\tMódulo "${Módulo}" atualizado`)))
	nocache('./configs.js', Módulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(`\tMódulo "${Módulo}" atualizado!`)))
	nocache('./app.js', Módulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(`\tMódulo "${Módulo}" atualizado!`)))
...
let participant = mek.key.participant;
        ...
        require("./app.js")(pl, m, messages, store)
        } catch (err) {
            console.log(err)
        }
    })
...
await pl.sendMessage(mdata.id,{text: '*Olha quem deu as cara por aqui,tchalzinho*'})
...
num = manu.participants[0]
pl.sendMessage(mdata.id, {text: 'OSH FAKE AKI NÃO *'})
...
Lft = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/d460e086f9f9bf6b04e17.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
	           ...
	           const members = metadata.participants.length
                let unicorndoc = {key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "916909137213-1604595598@g.us"}, "message": {orderMessage: {itemCount: 9999999,status: 200, thumbnail: Wlcm, surface: 200, message: `Jesus cristo reina`, orderTitle: 'PEDRO', sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true} //message: `${metadata.subject}`
                xbody = `_Olá @${Xnome.split("@")[0]} seja bem vindo(a) ao grupo ${metadata.subject}. Clique no botão abaixo ou digite *${prefix}menu* para ver em que posso te ajudar!_`

let buttons = [
{buttonId: `${prefix}menu`, buttonText: {displayText: 'Acessar o menu'}, type: 1},
]
let buttonMessage = {
image: wm_fatih,
Thumbnail: sai_kk,
...
headerType: 4,
                ...
                pl.sendMessage(manu.id, buttonMessage, { quoted: unicorndoc 
                    ...
                    xbody = `TCHAUZIN @${nome.split("@")[0]} vê se não dá motivos da próxima vez se é que vai ter uma próxima vez🤭`

let buttons = [
{buttonId: `KSKSKS`, buttonText: {displayText: '😭 foi em bora'}, type: 1},
{buttonId: `kakakak`, buttonText: {displayText: '⚘️ já foi tarde!'}, type: 1}
]
let buttonMessage = {
image: wm_fatih,
Thumbnail: sai_kk,
